wait_for_mysql() {
  ...
  while ! docker exec blog_db_1 sh -c 'mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" -e "SELECT 1;"' >/dev/null; do
    sleep 3
...
if [ ! -z "$input" ] && ( [ "$input" = "Y" ] || [ "$input" = "y" ] ); then
  ...
  docker exec -i blog_db_1 sh -c 'exec mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD"' < "$dump_file"
fi
...
chown -R www-data:www-data "${DISK_ROOT}/blog/wp-content/"
